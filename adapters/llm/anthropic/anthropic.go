...
func (c *Client) Classify(ctx context.Context, content, signalType string, categories []string) (*compiler.LLMResult, error) {
	...
	if math.IsNaN(result.Confidence) || math.IsInf(result.Confidence, 0) || result.Confidence < 0 || result.Confidence > 1 {
		return nil, fmt.Errorf("anthropic: confidence %f out of range [0,1]", result.Confidence)
	}
	...
	if !valid {
		return nil, fmt.Errorf("anthropic: LLM returned unknown category %q", result.Category)
	}
